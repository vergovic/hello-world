...
export const BUpdate = () => {
  ...
  const as = useAppSelector(state => state.myappms.a.entities);
  const bEntity = useAppSelector(state => state.myappms.b.entity);
  const loading = useAppSelector(state => state.myappms.b.loading);
  const updating = useAppSelector(state => state.myappms.b.updating);
  const updateSuccess = useAppSelector(state => state.myappms.b.updateSuccess);
  ...
  return (
    <div>
      <Row className="justify-content-center">
        <Col md="8">
          <h2 id="myAppMsApp.b.home.createOrEditLabel" data-cy="BCreateUpdateHeading">
            <Translate contentKey="myAppMsApp.b.home.createOrEditLabel">Create or edit a B</Translate>
          </h2>
        </Col>
      </Row>
      <Row className="justify-content-center">
        <Col md="8">
          {loading ? (
            ...
            <ValidatedForm defaultValues={defaultValues()} onSubmit={saveEntity}>
              ...
              <ValidatedField id="b-a" name="a" data-cy="a" label={translate('myAppMsApp.b.a')} type="select">
                <option value="" key="0" />
                {as
                  ? as.map(otherEntity => (
